...
WITH main_t AS (
    ...
    /* ----------------------------------------------------------
       ...
       SqlBuilder injects: AND inc.NUM_FASE_INCID IN (:lstOpenStages)
       and omits this entire branch when :lstOpenStages is empty.
    ---------------------------------------------------------- */
    SELECT
        inc.CONT_INCIDENCIA,
        cz.NOM_ZONA                                                    AS opco,
...
SELECT
    ...
    CASE WHEN t.restore_minus_etr_minutes < -90 THEN 'Y' ELSE ' ' END AS etr_set_too_late,
    CASE WHEN t.restore_minus_etr_minutes >  15 THEN 'Y' ELSE ' ' END AS etr_set_too_early,
